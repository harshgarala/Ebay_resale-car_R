...
par(mfrow=c(2,1))
plot(prop.table(table(data2$brand)))
plot(prop.table(table(data3$brand)))
...
# Futher considering price less than 50k
...
data4$Driven <- ifelse(data4$kilometer<50000,"Less",ifelse(data4$kilometer<100000,"Moderate","More"))
...
pr <- lm(price~Driven+km.per.year+gearbox+fuelType+notRepairedDamage,data=data4)
summary(pr)   
# p-value << 0.05
# R-squared = 0.402
...
pr1 <- lm(price~kilometer+age.years+notRepairedDamage+brand+powerPS+fuelType+vehicleType,data=data4)
summary(pr1)
# p-value << 0.5
# R-squared = 0.678
...
train.pr1 <- lm(price~kilometer+age.years+notRepairedDamage+brand+powerPS+fuelType+vehicleType,data=train.df)
summary(train.pr1)
par(mfrow=c(2,2))
...
compare <- data.frame(cbind(actual=test.df$price,predicted=pred.price))
